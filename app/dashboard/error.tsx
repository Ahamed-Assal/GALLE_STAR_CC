...
export default function DashboardError({
  ...
  useEffect(() => {
    console.error("Dashboard error:", error.message, error.digest, error.stack);
  }, [error]);

  return (
    <div className="mx-auto max-w-md space-y-6 py-16 text-center">
      <h1 className="text-2xl font-bold text-red-600 dark:text-red-400">Dashboard error</h1>
      <p className="text-sm text-slate-600 dark:text-slate-400">
        {process.env.NODE_ENV === "development"
          ? error.message
          : "Failed to load dashboard. This may be a database connection issue."}
      </p>
